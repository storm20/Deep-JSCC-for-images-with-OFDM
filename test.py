...
if __name__ == '__main__':
    ...
    for j in range (SNR_list.shape[0]):
        PSNR_list = []
        SSIM_list = []
        opt.SNR = SNR_list[j]
        # opt.SNR = 25
        print("Current test SNR: ", opt.SNR)
        for i, data in enumerate(dataset):
            ...
            if i%100 == 0:
                print(i)
        # print(len(PSNR_list))
        data = [opt.SNR,np.mean(PSNR_list)]
        header = ['SNR', 'PSNR'] 

        with open('test_results_0SNR_AWGN.csv', 'a', encoding='UTF8',newline='') as f:
            writer = csv.writer(f)
            if j == 0:
